...
import pickle
import warnings
from QMemory import QState, QMemory
...
class TetrisRaceQLearningAgent:
    ...
    def act(self, state, action, reward, state_):
        ...
        if reward == 0:
            ...
            if q_state:
                ...
                q_state.setWeightActR(-1)
                q_state.setWeightActL(-1)
                nr_state = self.q_memory.getNextRightState(q_state)
                nl_state = self.q_memory.getNextLeftState(q_state)
                nr_state.setWeightActL(-1)
                nr_state.setWeightActR(-1)
                nl_state.setWeightActR(-1)
                nl_state.setWeightActL(-1)

                if action == 1:
                    p_state = self.q_memory.getNextLeftState(nr_state)
                    p_state.setWeightActR(-1)
                    print(p_state.toString())
                else:
                    p_state = self.q_memory.getNextRightState(nl_state)
    ...
    def check_state_exist(self, state, action=-1):
        ...
        if action == -1:
            action = self.action
        for item in self.q_memory.listMemory:
            if item.getStateX() == state[0] and item.getStateY() == state[1]:
                return item
        return False
...
class Controler:
    def __init__(self, parent_mode = True , episodes_num = 10000, global_env = []):
        ...
        if parent_mode == False:
            ...
            env.__init__(walls_num=24, walls_spread=16, episodes_to_run=100000, smooth_car_step=10,
                         car_spawn='Center')
